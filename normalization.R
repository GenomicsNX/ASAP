      ...
      data.warnings <- c(data.warnings, paste0("It's not possible to perform batch effect correction with only one batch across all samples [", as.character(unique(data.batch)), "]. Batch effect correction was skipped."))
        ...
        for(batch in unique(data.batch)) {
          ...
          if(length(no.variance.genes) != 0){
            data.warnings <- c(data.warnings, paste0(length(no.variance.genes), " gene(s) have no variance (var = 0) in batch ", batch,". ComBat (batch effect correction method) cannot be ran with zero-variance genes. They are removed from the dataset [", paste(rownames(data.out)[no.variance.genes], collapse = ", "), "]"))
