...
if(is.null(data.batch)){
  ...
  data.design <- model.matrix(~data.batch[colnames(data.norm),]+data.group[colnames(data.norm),])
}
...
if(algorithm=="limma"){
  ...
  } else { # Do not use batch effect correction on data already normalized
  ...
  #data.group <- read.table(group.file, sep="\t", header=F, row.names=1, check.names=F, stringsAsFactors=F)
  nb.cores <- as.integer(nb.cores)
  data.model.ifm <- scde.error.models(counts=data.norm, groups=data.group[colnames(data.norm),1], n.cores=nb.cores, threshold.segmentation = TRUE, save.crossfit.plots = FALSE, save.model.plots = FALSE, verbose = 1)
